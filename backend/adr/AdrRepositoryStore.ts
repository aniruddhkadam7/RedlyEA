...
const notifyAdrsChanged = () => {
  try {
    if (typeof window !== 'undefined')
      window.dispatchEvent(new Event('ea:adrsChanged'));
  } catch {
    // Best-effort only.
  }
};
...
const makeAdrId = (): string => {
  ...
  if (typeof anyCrypto.randomUUID === 'function')
    return `adr_${anyCrypto.randomUUID()}`;
...
export function getAdrRepository(): AdrRepository {
  ...
  if (!projectId)
    throw new Error(
      'No active project. Create/select a project before managing ADRs.',
    );
