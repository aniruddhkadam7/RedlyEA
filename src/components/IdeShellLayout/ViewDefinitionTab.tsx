...
import {
  ENTERPRISE_VIEW_GOVERNANCE_POLICY,
  evaluateViewGovernance,
} from '../../../backend/views/ViewGovernance';
import { getViewRepository } from '../../../backend/views/ViewRepositoryStore';
import {
  type ResolvedViewData,
  viewResolver,
} from '../../../backend/views/ViewResolver';
...
const layoutHintForView = (
  ...
  const fallback = {
    ...
    orientation: 'TopDown' as Orientation,
  };
...
const ViewDefinitionTab: React.FC<ViewDefinitionTabProps> = ({ viewId }) => {
  ...
  const [lifecycleFilter, setLifecycleFilter] =
    React.useState<LifecycleFilter>('Both');
  const [relationshipFilters, setRelationshipFilters] = React.useState<
    Record<string, boolean>
  >({
    SUPPORTED_BY: true,
    REALIZED_BY: true,
  ...
  const applyImpactHighlight = React.useCallback(() => {
    ...
    cy.batch(() => {
      ...
      const elementIds = Array.from(
        new Set(
          (detail.orderedElementIds ?? []).map(normalizeId).filter(Boolean),
        ),
      );
      const relationshipIds = Array.from(
        new Set(
          (detail.orderedRelationshipIds ?? [])
            .map(normalizeId)
            .filter(Boolean),
        ),
      );
      ...
      const hasAny =
        elementIds.some((id) => cy.$id(id).nonempty()) ||
        relationshipIds.some((id) => cy.$id(id).nonempty());
      if (!hasAny) return;
  ...
  const applyFilters = React.useCallback(() => {
    ...
    cy.batch(() => {
      cy.nodes().forEach((node) => {
        ...
        const lifecycleVisible =
          lifecycleFilter === 'Both' || lifecycleState === lifecycleFilter;
  ...
  const _viewpointInfo = React.useMemo(() => {
    if (!view) return null;
  ...
  const noElementsForViewpoint = Boolean(
    resolved && resolved.elements.length === 0,
  );

  const governance = React.useMemo(() => {
    if (!view) return null;
    return evaluateViewGovernance(view, {
      resolvedElements: resolved?.elements ?? [],
    });
  }, [view, resolved]);

  const preventExternalDrop = React.useCallback(
  ...
  const horizonWindow = React.useMemo(
    ...
    <ProCard>
      ...
      <Alert
        ...
        description={
          <div>
            <div>
              Repository is the source of truth; Properties is the primary work
              surface.
            </div>
            <div>
              Auto-generated from repository with auto-layout; no free-form
              drawing or canvas-only creation.
            </div>
            <div>
              Adding elements means selecting existing repository items only;
              deleting a diagram never deletes elements.
            </div>
            <div>
              Relationships are created in Properties only. No lines,
              auto-connects, or canvas wiring here.
            </div>
            <div>
              Validation over visuals: governance rules still apply even if the
              diagram looks fine.
            </div>
          </div>
        }
      ...
      <Typography.Paragraph type="secondary" style={{ marginBottom: 12 }}>
        Read-only diagram lens (pan/zoom). Node move/resize is disabled; open
        Properties to edit truth.
      ...
      <div style={{ marginBottom: 12 }}>
        <ArchitectureReviewPanel
          subjectKind="View"
          subjectId={view.id}
          defaultReviewer={view.createdBy}
        />
      </div>
        ...
        items={[
          {
            ...
            children: (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                >
                  Filters adjust what is shown on this diagram only. They do not
                  change repository data, layout definitions, or relationships.
                </Typography.Paragraph>
                ...
                <Space direction="vertical" size={6}>
                  ...
                  <Checkbox
                    ...
                    onChange={(e) =>
                      setTypeFilters((prev) => ({
                        ...prev,
                        BusinessService: e.target.checked,
                      }))
                    }
                  >
                    Business Service
          ...
          description={
            <div>
              <Typography.Paragraph
                ...
                type="secondary"
              >
                Policy: maxDepth ≤ {ENTERPRISE_VIEW_GOVERNANCE_POLICY.maxDepth}.
                Warnings do not block usage.
              </Typography.Paragraph>
      ...
      <Typography.Paragraph type="secondary" style={{ marginTop: 8 }}>
        Diagrams are disposable lenses. Deleting this diagram does not delete
        any repository elements or relationships.
      </Typography.Paragraph>
      ...
      <Descriptions bordered size="small" column={1}>
        ...
        <Descriptions.Item label="Architecture layer">
          {view.architectureLayer}
        </Descriptions.Item>
        <Descriptions.Item label="Root element type">
          {view.rootElementType ?? '(none)'}
        </Descriptions.Item>
        <Descriptions.Item label="Root element id">
          {view.rootElementId ?? '(none)'}
        </Descriptions.Item>
        <Descriptions.Item label="Max depth">
          {typeof view.maxDepth === 'number' ? view.maxDepth : '(none)'}
        </Descriptions.Item>
