...
const generateElementId = (type: ObjectType): string =>
  `${defaultIdPrefixForType(type)}${generateUUID()}`;

const areKeyArraysEqual = (left: React.Key[], right: React.Key[]): boolean => {
...
const ExplorerTree: React.FC = () => {
  ...
  const [changeTypeTarget, setChangeTypeTarget] = React.useState<{
    id: string;
    type: ObjectType;
    name: string;
  } | null>(null);
  ...
  const nodeMetaByKey = React.useMemo(() => {
    const map = new Map<
      string,
      { parent: string | null; hasChildren: boolean; data?: any }
    >();
  ...
  const guardAction = React.useCallback(
    (action: ExplorerAction, context?: string): boolean => {
  ...
  const commitRename = React.useCallback(() => {
    ...
    const sameTypeObjects = Array.from(eaRepository.objects.values()).filter(
      (o) =>
        o.type === obj.type && o.id !== obj.id && !isSoftDeleted(o.attributes),
    );
    const duplicate = sameTypeObjects.some(
      (o) => nameForObject(o).toLowerCase() === newName.toLowerCase(),
    );
    if (duplicate) {
      message.warning(
        `Another ${titleForObjectType(obj.type)} with this name already exists.`,
      );
    }
    ...
    auditObjectMutation({
      ...
      objectType: obj.type,
      before: beforeState,
      after: { ...beforeState, name: newName },
    });
  ...
  const commitChangeType = React.useCallback(() => {
    if (!changeTypeTarget || !changeTypeNewType || !eaRepository) {
    ...
    if (targetObj) {
      ...
      const updateRes = next.updateObjectAttributes(
        changeTypeTarget.id,
        {
    ...
    message.success(
      `Type changed to ${titleForObjectType(changeTypeNewType)}.`,
    );
  }, [
    actor,
    changeTypeNewType,
  ...
  const computeTargetOptions = React.useCallback(
    (relType: RelationshipType, source: { id: string; type: ObjectType }) => {
      if (!eaRepository) return [];
      ...
      return Array.from(eaRepository.objects.values())
        .filter(
          (o) => o.id !== source.id && (o.attributes as any)?._deleted !== true,
        )
        .filter((o) => {
          const toType = o.type as ObjectType;
          if (pairs?.length)
            return pairs.some((p) => p.from === source.type && p.to === toType);
  ...
  const saveRelationshipFromModal = React.useCallback(() => {
    ...
    emitExplorerEvent({
      ...
      fromId: relationshipSource.id,
      toId: selectedTargetId,
      relType: selectedRelationshipType,
      timestamp: new Date().toISOString(),
      actor,
    });
        ...
        case 'add-to-view': {
          ...
          if (obj) {
            setAddToViewTarget({
              ...
              type: action.elementType as ObjectType,
            });
  ...
  const handleSingleClick = React.useCallback(
    (key: string) => {
      ...
      switch (kind) {
        ...
        case 'setting': {
          const settingKey = (meta?.data as any)?.settingKey;
          if (
            settingKey === 'custom-metamodel-element-types' ||
            settingKey === 'custom-metamodel-relationship-types'
          ) {
            openWorkspaceTab({ type: 'metamodel-editor' });
          }
          break;
        }
        default:
  ...
  const handleDoubleClick = React.useCallback(
    (key: string) => {
      const meta = nodeMetaByKey.get(key);
      const data = meta?.data as
        | { elementId?: string; elementType?: string; viewId?: string }
        | undefined;
      const kind = classifyNodeKey(key, meta?.data);
          ...
          if (containingView) {
            ...
            openWorkspaceTab({
              ...
              elementName: nameForObject({
                id: elementId,
                attributes: eaRepository?.objects.get(elementId)?.attributes,
              }),
              elementType,
            });
          }
  ...
  const handleDragStart = React.useCallback(
    (event: React.DragEvent<HTMLSpanElement>, key: string, data: any) => {
      if (!data) return;
  ...
  const handleExpand = React.useCallback((nextExpandedKeys: React.Key[]) => {
    setExpandedKeys((prev) =>
      areKeyArraysEqual(prev, nextExpandedKeys) ? prev : nextExpandedKeys,
    );
  }, []);
        ...
        <Dropdown trigger={['contextMenu']} menu={menuForKey(k)}>
          ...
          >
            {fwTags.length > 0 ? (
              ...
              <span
                className={styles.explorerTreeLabel}
                style={showCreateButton ? { flex: 1 } : undefined}
              >
                {renderedTitle as any}
              </span>
            )}
            {showCreateButton && quickCreate && (
  ...
  return (
    <div className={styles.explorerTree}>
      <div className={styles.explorerTreeHeader}>
        {/* §9 Search Bar */}
        {searchVisible && (
          <div
            style={{
      ...
      <div className={styles.explorerTreeContainer}>
        ...
        <Tree
          ...
          draggable={{
            ...
            nodeDraggable: (node: any) => {
              const data = (node as any)?.data;
              return Boolean(
                data?.elementId || data?.viewId || data?.roadmapId,
              );
            },
          }}
      ...
      <Modal
        ...
        okButtonProps={{
          disabled:
            !changeTypeNewType || changeTypeNewType === changeTypeTarget?.type,
        }}
        destroyOnClose
      >
        {changeTypeTarget && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div>
              ...
              <Input
                value={titleForObjectType(changeTypeTarget.type)}
                disabled
                style={{ marginTop: 4 }}
              />
            </div>
            <div>
              ...
              <Select
                ...
                options={creatableTypeOptions.filter(
              ...
              <Select
                ...
                options={(() => {
                  ...
                  const allowed = (
                    ...
                    return Boolean(
                      RELATIONSHIP_TYPE_DEFINITIONS[t]?.fromTypes.includes(
                        relationshipSource.type as ObjectType,
                      ),
                    );
                  }) as RelationshipType[];
...
const AuditTrailView: React.FC<{ objectId: string }> = ({ objectId }) => {
  ...
  if (entries.length === 0) {
    return (
      <Typography.Text type="secondary">
        No audit entries found for this object.
      </Typography.Text>
    );
  }
