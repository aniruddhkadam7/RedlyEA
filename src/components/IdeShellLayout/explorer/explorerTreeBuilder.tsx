...
export function buildExplorerTree(
  ...
  // ---------------------------------------------------------------------------
  // CUSTOM (Architect Mode) — simplified explorer tree
  // ---------------------------------------------------------------------------
  if (metadata?.initializationMode === 'CUSTOM') {
    return buildCustomExplorerTree({
      workspaceName,
      objectsById,
      relationships,
      metadata,
  ...
  const businessNode: DataNode = collectionNode(
    ...
    [
      collectionNode(
        ...
        elementLeaves(objectsById, visCapabilities),
        {
          ...
          quickCreate: {
            defaultType: (visCapabilities[0] ?? 'Capability') as ObjectType,
            category: 'Business',
          },
        },
      ),
      collectionNode(
        ...
        elementLeaves(objectsById, visProcesses),
        {
          ...
          quickCreate: {
            defaultType: (visProcesses[0] ?? 'BusinessProcess') as ObjectType,
            category: 'Business',
          },
        },
      ),
      collectionNode(
        ...
        elementLeaves(objectsById, visActors),
        {
          ...
          quickCreate: {
            defaultType: (visActors[0] ?? 'Department') as ObjectType,
            category: 'Business',
          },
        },
      ),
    ],
    {
      ...
      quickCreate: {
        defaultType: 'BusinessProcess' as ObjectType,
        category: 'Business',
      },
    },
  );
  ...
  const applicationsNode: DataNode = collectionNode(
    ...
    [
      collectionNode(
        ...
        elementLeaves(objectsById, visApplications),
        {
          ...
          quickCreate: {
            defaultType: (visApplications[0] ?? 'Application') as ObjectType,
      ...
      collectionNode(
        ...
        elementLeaves(objectsById, visAPIs),
        {
          ...
          quickCreate: {
            defaultType: (visAPIs[0] ?? 'API') as ObjectType,
            category: 'Application',
          },
        },
      ),
      collectionNode(
        ...
        elementLeaves(objectsById, visServices),
        {
          ...
          quickCreate: {
            defaultType: (visServices[0] ?? 'ApplicationService') as ObjectType,
            category: 'Application',
          },
        },
      ),
    ],
    {
      ...
      quickCreate: {
        defaultType: 'Application' as ObjectType,
        category: 'Application',
      },
    },
  );
    ...
    [
      collectionNode(
        ...
        elementLeaves(objectsById, visDataEntities),
        {
          ...
          quickCreate: {
            defaultType: 'Database' as ObjectType,
            category: 'Data',
          },
        },
      ),
      collectionNode(
        ...
        elementLeaves(objectsById, visDataStores),
        {
          ...
          quickCreate: {
            defaultType: (visDataStores[0] ?? 'Storage') as ObjectType,
            category: 'Data',
          },
        },
      ),
  ...
  const technologyNode: DataNode = collectionNode(
    ...
    [
      collectionNode(
        ...
        elementLeaves(objectsById, visInfrastructure),
        {
          ...
          quickCreate: {
            defaultType: (visInfrastructure[0] ?? 'Node') as ObjectType,
            category: 'Technology',
          },
        },
      ),
      collectionNode(
        ...
        elementLeaves(objectsById, visNetwork),
        {
          ...
          quickCreate: {
            defaultType: (visNetwork[0] ?? 'Network') as ObjectType,
            category: 'Technology',
          },
        },
      ),
      collectionNode(
        ...
        elementLeaves(objectsById, visCloud),
        {
          ...
          quickCreate: {
            defaultType: (visCloud[0] ?? 'CloudService') as ObjectType,
            category: 'Technology',
          },
        },
      ),
    ],
    {
      ...
      quickCreate: {
        defaultType: 'Node' as ObjectType,
        category: 'Technology',
      },
    },
  );
  ...
  const baselinesIcon = iconAsset('/icons/explorer/baseline.svg', 'baselines');
...
// ---------------------------------------------------------------------------
// CUSTOM (Architect Mode) Explorer Tree
// ---------------------------------------------------------------------------
//
// Minimal tree for Custom initialization mode:
//   Repository
//   ├── Diagrams
//   ├── Elements
//   ├── Relationships
...
function buildCustomExplorerTree(input: {
  ...
  // Diagrams node — all saved views (flat, no category split)
  const diagramsNode: DataNode = collectionNode(
    EXPLORER_KEYS.architectures, // reuse key slot
    'Diagrams',
    diagramIcon,
    savedViews.length > 0
      ? savedViews.map(viewLeaf)
      : [emptyPlaceholder('custom:diagrams', 'No diagrams yet')],
    { quickCreate: { action: 'create-diagram' } },
  );

  // Elements node — all non-deleted objects, grouped dynamically
  const liveObjects = Array.from(objectsById.values()).filter(
  ...
  const elementLeafNodes: DataNode[] = liveObjects.map((o) => ({
    key: EXPLORER_KEYS.element(o.id),
    title: nameForObject(o),
    icon: iconForObjectType(o.type),
    isLeaf: true,
    className: 'explorer-node explorer-node-component explorer-node-element',
    data: { elementId: o.id, elementType: o.type },
  }));

  const elementsNode: DataNode = collectionNode(
    'custom:elements',
    'Elements',
    componentIcon,
    elementLeafNodes.length > 0
      ? elementLeafNodes
      : [emptyPlaceholder('custom:elements', 'No elements yet')],
  );

  // Relationships node
  const relationshipNodes: DataNode[] = relationships
    .slice(0, 200)
    .map((r) => relationshipLeaf(r, objectsById));

  const relationshipsNode: DataNode = collectionNode(
    'custom:relationships',
    'Relationships',
    connectionIcon,
    relationshipNodes.length > 0
      ? relationshipNodes
      : [emptyPlaceholder('custom:relationships', 'No relationships yet')],
    { quickCreate: { action: 'create-relationship' } },
  );

  // Metamodel node (simplified — opens metamodel editor)
  const metamodelNode: DataNode = collectionNode(
    EXPLORER_KEYS.metamodel,
    'Metamodel',
    metamodelIcon,
    [
      {
        key: 'custom:metamodel:element-types',
        title: 'Element Types',
        icon: metamodelIcon,
        isLeaf: true,
        data: { settingKey: 'custom-metamodel-element-types' },
      },
      {
        key: 'custom:metamodel:relationship-types',
        title: 'Relationship Types',
        icon: connectionIcon,
        isLeaf: true,
        data: { settingKey: 'custom-metamodel-relationship-types' },
      },
    ],
  );
  ...
  const rootNode: DataNode = {
    ...collectionNode(
      ...
      `${workspaceName}.ea`,
      folderIcon,
      [diagramsNode, elementsNode, relationshipsNode, metamodelNode],
    ),
    className: 'explorer-level-root',
  };
