...
const IdeMenuBar: React.FC = () => {
  ...
  // .Redly Save As
  // -----------------------------------------------------------------------
  const buildRedlyExportSource =
    React.useCallback((): RedlyExportSource | null => {
      if (!eaRepository || !metadata) return null;
    ...
    React.useCallback(
      async (e) => {
        ...
        if (!file.name.toLowerCase().endsWith(".redly")) {
          message.warning({
            content:
              "Unsupported file type. Please choose a .Redly repository file.",
            duration: 5,
          });
          return;
        }
  ...
  const handleCheckForUpdates = React.useCallback(async () => {
    console.log("[IDE] Help > Check for Updates");

    // Check if running in desktop app with updater API
    if (!window.eaDesktop?.updater) {
      Modal.info({
        title: "Updates",
        content: "Auto-updates are available in the desktop app.",
      });
      return;
    }
          ...
          content: (
            <div>
              <p>
                ...
                <strong>v{result.updateInfo.version}</strong>
              </p>
              <p>
                Current version: <strong>v{currentVersion}</strong>
              </p>
              {result.updateInfo.releaseDate && (
                <p style={{ color: "#888", fontSize: 12 }}>
                  Released:{" "}
                  {new Date(result.updateInfo.releaseDate).toLocaleDateString()}
                </p>
              )}
            ...
            try {
              ...
              if (dlResult?.ok) {
                Modal.confirm({
                  ...
                  onOk: () => {
                    Modal.destroyAll();
                    // Trigger the install (main process quits and installs)
