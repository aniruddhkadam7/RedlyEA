...
): ImpactSeverityScore['severityLabel'] => {
...
export function analyzeImpactLocally(args: {
  ...
  const maxDepth =
    typeof request.maxDepth === 'number' && request.maxDepth > 0
      ? Math.trunc(request.maxDepth)
      : 5;

  const allowedRelationshipTypes = new Set(
    (request.includedRelationshipTypes ?? [])
      .map((t) => normalizeId(t))
      .filter((t) => t.length > 0),
  );
  ...
  for (const r of repo.relationships ?? []) {
    ...
    if (!type) continue;
    if (
      allowedRelationshipTypes.size > 0 &&
      !allowedRelationshipTypes.has(type)
    )
      continue;
    ...
    if (
      !relationshipAllowedByMetamodel(type as RelationshipType, fromObj, toObj)
    )
      continue;
  ...
  const rankedImpacts: ImpactRankedElement[] = evidence
    ...
    .sort((a, b) => {
      ...
      if (as !== 0) return -as;
      if (a.evidence.maxDepthObserved !== b.evidence.maxDepthObserved)
        return b.evidence.maxDepthObserved - a.evidence.maxDepthObserved;
      if (
        a.evidence.totalPathsAffectingElement !==
        b.evidence.totalPathsAffectingElement
      ) {
        return (
          b.evidence.totalPathsAffectingElement -
          a.evidence.totalPathsAffectingElement
  ...
  const severityBreakdown: ImpactSummary['severityBreakdown'] = {
    High: 0,
    Medium: 0,
    Low: 0,
  };
  ...
  const impactSummary: ImpactSummary = {
    ...
    maxDependencyDepthObserved: Math.max(
      0,
      ...evidence.map((e) => e.maxDepthObserved),
    ),
