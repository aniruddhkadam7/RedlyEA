...
describe('Custom framework config', () => {
  ...
  test('normalizes enabledRelationshipTypes (dedupe + sort + filter invalid)', () => {
    ...
    expect(cfg.enabledRelationshipTypes).toEqual([
      'EXPOSES',
      'OWNS',
      'REALIZES',
      'SERVED_BY',
  ...
  test('getCustomMetaModelConfig defaults to Custom core seed', () => {
    ...
    expect(
      getCustomMetaModelConfig(undefined).enabledRelationshipTypes,
    ).toEqual([
      'DEPENDS_ON',
      'DEPLOYED_ON',
      'EXPOSES',
  ...
  test('missing Custom config falls back to seed, explicit empty stays empty', () => {
    expect(
      getCustomMetaModelConfig({ custom: {} as any }).enabledObjectTypes.length,
    ).toBeGreaterThan(0);
    expect(
      getCustomMetaModelConfig({
        custom: { enabledObjectTypes: [], enabledRelationshipTypes: [] },
      }).enabledObjectTypes,
    ).toEqual([]);
  });
  ...
  test('Custom modeling disabled until at least one type enabled', () => {
    expect(
      isCustomFrameworkModelingEnabled('Custom', {
        custom: { enabledObjectTypes: [] },
      }),
    ).toBe(false);
    expect(
      isCustomFrameworkModelingEnabled('Custom', {
        custom: { enabledObjectTypes: ['Application'] },
      }),
    ).toBe(true);
  });
  ...
  test('object type enablement is enforced only for Custom', () => {
    ...
    expect(
      isObjectTypeEnabledForFramework('ArchiMate', null, 'Capability'),
    ).toBe(true);
