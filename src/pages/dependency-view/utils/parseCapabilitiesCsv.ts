...
export type CapabilitiesCsvParseSuccess = {
  ok: true;
  capabilities: CapabilitiesCsvRow[];
};

export type CapabilitiesCsvParseFailure = {
  ok: false;
  errors: string[];
};
...
export function parseAndValidateCapabilitiesCsv(
  csvText: string,
  opts: { repository: Pick<EaRepository, 'objects'> },
): CapabilitiesCsvParseResult {
  const errors: string[] = [];
  ...
  for (let r = 1; r < rows.length; r += 1) {
    ...
    if (!rawType) {
      errors.push(`Row ${displayRow}: type is required.`);
    } else if (!isValidObjectType(rawType)) {
      errors.push(`Row ${displayRow}: type is invalid (got "${rawType}").`);
    }
