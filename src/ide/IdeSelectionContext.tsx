...
export type IdeSelectionContextValue = {
  ...
  setSelectedElement: (
    next: { id: string; type: string; source?: SelectionSource } | null,
  ) => void;
...
const IdeSelectionContext = React.createContext<
  IdeSelectionContextValue | undefined
>(undefined);

export const IdeSelectionProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  const setSelection = React.useCallback(
    (next: { kind: IdeSelectionKind; keys: string[] }) => {
  ...
  const setActiveDocument = React.useCallback(
    (next: { kind: 'route' | 'workspace'; key: string }) => {
      setSelectionState((prev) => ({
        ...prev,
  ...
  const setSelectedElement = React.useCallback(
    (next: { id: string; type: string; source?: SelectionSource } | null) => {
      setSelectionState((prev) => ({
        ...
        activeImpactElementId:
          next?.source === 'ImpactAnalysis' && next ? next.id : null,
        activeImpactElementType:
          next?.source === 'ImpactAnalysis' && next ? next.type : null,
      }));
    },
    [],
  );
  ...
  const setActiveImpactElement = React.useCallback(
    (next: { id: string; type: string } | null) => {
  ...
  return (
    <IdeSelectionContext.Provider value={value}>
      {children}
...
export function useIdeSelection(): IdeSelectionContextValue {
  ...
  if (!ctx)
    throw new Error('useIdeSelection must be used within IdeSelectionProvider');
  return ctx;
}
