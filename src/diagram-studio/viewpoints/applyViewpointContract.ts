 ...
 */
export function applyViewpointContract(
  input: ViewpointContractInput,
): ViewpointContractResult {
  ...
  const allowedElementIds = new Set(
    filteredElements.map((e) => normalize(e.id)),
  );

  const filteredRelationships = (input.relationships ?? []).filter(
    (relationship) => {
      if (
        !allowedRelationshipTypes.has(normalize(relationship.relationshipType))
      )
        return false;
      if (!allowedElementTypes.has(normalize(relationship.sourceElementType)))
        return false;
      if (!allowedElementTypes.has(normalize(relationship.targetElementType)))
        return false;
