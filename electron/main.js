...
if (process.platform === "win32") {
  ...
  app.setAppUserModelId("com.redlyea.desktop");
}
...
const resolveFileTypeIconPath = () => {
  ...
  try {
    ...
    const tempIcoPath = path.join(app.getPath("temp"), "redlyea-file.ico");
...
const registerEaFileTypes = () => {
  ...
  try {
    ...
    registerFileType("eapkg", "RedlyEA.EAPkg", "EA Repository Package");
    registerFileType("eaproj", "RedlyEA.EAProj", "EA Project");
...
const resolveAppIcon = () => {
  ...
  const publicPngCandidates = [
    path.join(appBasePath, "public", "icons", "icon-512x512.png"),
    path.join(appBasePath, "public", "icons", "icon-192x192.png"),
    path.join(appBasePath, "public", "icons", "icon-128x128.png"),
  ];

  // 1. Prefer Windows ICO from build/icons (generated by desktop:icons)
  if (process.platform === "win32" && fs.existsSync(appIcoPath)) {
    console.log("[EA] Using Windows ICO icon:", appIcoPath);
    return appIcoPath;
  }
  // 2. PNG from build/icons (generated by desktop:icons)
  ...
  // 4. Source PNGs from public/icons/ (always committed to git)
  for (const pngPath of publicPngCandidates) {
    if (fs.existsSync(pngPath)) {
      // On Windows, convert PNG → temp ICO for proper taskbar/sidebar icon
      if (process.platform === "win32") {
        try {
          const pngBuf = fs.readFileSync(pngPath);
          const icoBuf = buildIcoFromPngBuffer(pngBuf);
          if (icoBuf) {
            const tmpIco = path.join(app.getPath("temp"), "ea-app-icon.ico");
            fs.writeFileSync(tmpIco, icoBuf);
            console.log(
              "[EA] Created temp ICO from PNG:",
              pngPath,
              "→",
              tmpIco,
            );
          ...
          console.warn("[EA] Failed to create temp ICO:", e.message);
        }
      }
      console.log("[EA] Using public icons PNG:", pngPath);
      return pngPath;
    }
  }

  console.warn("[EA] No app icon found. Checked:");
  console.warn("  - ICO:", appIcoPath);
  console.warn("  - PNG:", appIconPath);
  console.warn("  - Fallback:", fallbackIco);
  publicPngCandidates.forEach((p) => console.warn("  - Public:", p));
    ...
    callback({
      responseHeaders: {
        ...
        "Content-Security-Policy": [
          "default-src 'self' 'unsafe-inline' 'unsafe-eval' data: blob: file: http: https:;",
        ],
      },
    });
  });
